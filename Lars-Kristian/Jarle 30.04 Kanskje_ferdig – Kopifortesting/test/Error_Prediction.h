...
typedef struct {
	uint16_t fans[NUM_FANS];
	uint8_t long_term[NUM_FANS];
	uint8_t short_term[NUM_FANS];
} FanInfo;

// Function init
void RTC_init(void);
FanInfo average_values_for_tach(void);
void predict_error();
...
FanInfo average_values_for_tach()
{
	...
	FanInfo average_read_values[NUM_FANS];
	average_read_values[0] = average_read_value_fan1;
	average_read_values[1] = average_read_value_fan2;
	
	// FJERNES, BARE FOR TESTING
	TCA0.SPLIT.LCMP1 = DUTY_CYCLE_MIN_VALUE;
	
	return average_read_values[NUM_FANS];
}
...
{	
	...
	FanInfo current_RPM_values[NUM_FANS];
